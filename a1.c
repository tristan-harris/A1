...
void normal_mode_entry();
void normal_mode_input(int input);
void normal_mode_exit();

void insert_mode_entry();
void insert_mode_input(int input);
void insert_mode_exit();

void search_mode_entry();
void search_mode_input(int input);
void search_mode_exit();
...
EditorMode search_mode = {
  .entry_fn = search_mode_entry,
  .input_fn = search_mode_input,
  .exit_fn = search_mode_exit,
  .name = "SEARCH"
};
...
void transition_mode(EditorMode *new_mode) {
  if (editor_state.mode != NULL) { editor_state.mode->exit_fn(); }
  editor_state.mode = new_mode;
  if (editor_state.mode != NULL) { editor_state.mode->entry_fn(); }
}
...
void init_editor() {
  ...
  transition_mode(&normal_mode); // start editor in normal mode
}
