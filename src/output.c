...
static void editor_scroll_render_update(void) {
    int y_position;
...
static void editor_add_row_end(AppendBuffer *ab) {
...
// utility function for editor_draw_rows()
static bool editor_at_block_cursor(int cur_row, int cur_col) {
    bool at_row = false;
    bool at_col = false;
...
static void editor_draw_rows(AppendBuffer *ab) {
    char esc_seq_buf[15];
...
static void editor_add_to_status_bar_buffer(char buffer[], size_t max_len,
                                            int *buffer_len,
                                            int *buffer_render_len,
                                            const char *fmt, ...) {
...
static void editor_draw_status_bar(AppendBuffer *ab) {
    ...
    editor_get_scroll_percentage(scroll_percent, sizeof(scroll_percent));
...
static void editor_draw_bottom_bar(AppendBuffer *ab) {
    ab_append(ab, "\x1b[K", 3); // erase from active position to end of line
